...
app.config['MAIL_USERNAME'] = 'jethvaronak98@gmail.com'  # Set to your email if you want email features
app.config['MAIL_PASSWORD'] = 'rxnlgufjhvlcmokr'  # Set to your app password
...
# --- MOBILE VERIFICATION TOKENS (In-memory) ---
...
def current_user():
    ...
    if not uid:
        return None
    return fetchone("SELECT id,name,email,is_admin,face_image FROM users WHERE id=%s", (uid,))
...
def register():
    ...
    if request.method == "POST":
        ...
        if captured_data:
            ...
            flash("Opening webcam to capture your face.", "info")
            enc_bytes, cropped_bgr, err = capture_face_encoding_server(window_title="Register: capture face")
            if enc_bytes is None:
                flash(f"Face capture failed: {err}", "danger")
                return render_template("auth_register.html", name=pre_name, email=pre_email)
        pw_hash = generate_password_hash(password)
        uid = execute(
            "INSERT INTO users (name, email, password_hash, face_encoding) VALUES (%s,%s,%s,%s)",
            (name, email, pw_hash, enc_bytes)
        )
        if cropped_bgr is not None:
            rel_path = _save_face_image_bytes(uid, cropped_bgr)
            if rel_path:
                try:
                    execute("UPDATE users SET face_image=%s WHERE id=%s", (rel_path, uid))
                except Exception:
                    pass
        # >>>>>>>>>> CHANGED: Redirect to complete profile instead of logging in <<<<<<<<<<
...
def complete_profile():
    ...
    if request.method == "POST":
        ...
        execute("""
            UPDATE users 
            SET mobile=%s, address_line1=%s, address_line2=%s, city=%s, state=%s, pincode=%s, profile_complete=TRUE 
            WHERE id=%s
        """, (mobile, address_line1, address_line2, city, state, pincode, user_id))
...
def login():
    if request.method == "POST":
        ...
        # Use server-side webcam for face verification (fallback)
